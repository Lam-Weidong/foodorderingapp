...
def index_page():
    #Index page   
    return 'Order Shop Api \n '+'please login by /login/customer_id/password', 200

@app.route('/login/<customer_id>/<password>', methods=['GET'])
def login(customer_id,password):
    test = mycol.find_one({"customer_id": customer_id,"password":password})
    if test:
        access_token = create_access_token(identity=customer_id)
        session['token'] = access_token
        return jsonify(message="Login Succeeded!", access_token=access_token), 201
    else:
        return jsonify(message="Bad customer ID or Password"), 401
...
#editing cart
@app.route('/cart/<customer_id>', methods=['PATCH',"get"])
